...
proc autoMagic(level: string = "low", skipProgressBar:bool = false, displayTable:bool = false, output: string = "case-1", quiet: bool = false, timeline: string) =
    ...
    let cmd3 = ListDomainsCmd(name:"list-domains(detailed)", displayTable:displayTable, timeline:timeline, output:output & "/ListDomainsDetailed.txt", includeSubdomains:true, includeWorkstations:true)

    # list-hashes -t ../hayabusa/timeline.jsonl -l <level> -o case-1/hashes
    let cmd4 = ListHashesCmd(name:"list-hashes", displayTable:displayTable, timeline:timeline, level:level, output:output & "/ListHashes")

    # list-ip-addresses -t ../hayabusa/timeline.jsonl -o case-1/IP-Addresses.txt
    let cmd5 = ListIpAddressesCmd(name:"list-ip-addresses", displayTable:displayTable,  timeline:timeline, output:output & "/ListIP-Addresses.txt")

    # stack-cmdlines -t ../hayabusa/timeline.jsonl --level <leve> -o case-1/cmdlines.csv
    let cmd6 = StackCmdlineCmd(name:"stack-cmdlines", level:level, displayTable:displayTable, timeline:timeline, output:output & "/StackCmdlines.csv")

    # stack-computers -t ../hayabusa/timeline.jsonl --level <level> -o case-1/TargetComputers.csv
    let cmd7 = StackComputersCmd(name: "stack-computers", level:level, displayTable:displayTable, timeline:timeline, output:output & "/StackTargetComputers.csv")

    # stack-computers -t ../hayabusa/timeline.jsonl --level <level> --sourceComputers -o case-1/SourceComputers.csv
    let cmd8 = StackComputersCmd(name: "stack-computers", level:level, displayTable:displayTable, timeline:timeline, output:output & "/StackSourceComputers.csv", sourceComputers:true)

    # stack-dns -t ../hayabusa/timeline.jsonl --level <level>  -o case-1/DNS.csv
    let cmd9 = StackDNSCmd(name:"stack-dns", level:level, displayTable:displayTable, timeline:timeline, output:output & "/StackDNS.csv")

    # stack-ip-addresses -t ../hayabusa/timeline.jsonl --level <level> -o case-1/SourceIP-Addresses.csv
    let cmd10 = StackIpAddressesCmd(name: "stack-ip-addresses(source)", level:level, displayTable:displayTable, timeline:timeline, output:output & "/StackSourceIP-Addresses.csv")
    ...
    let cmd14 = StackServicesCmd(name:"stack-services", level:level, displayTable:displayTable, timeline:timeline, output:output & "/StackServices.csv")

    # stack-tasks -t ../hayabusa/timeline.jsonl --level <level> -o case-1/Tasks.csv
    let cmd15 = StackTasksCmd(name:"stack-tasks", level:level, displayTable:displayTable, timeline:timeline, output:output & "/StackTasks.csv")

    # stack-users -t ../hayabusa/timeline.jsonl --level <level> -o case-1/TargetUsers.csv
    let cmd16 = StackUsersCmd(name:"stack-users(target)", level:level, displayTable:displayTable, timeline:timeline, output:output & "/StackTargetUsers.csv" , filterSystemAccounts:true, filterComputerAccounts:true)
    ...
    let cmd17 = StackUsersCmd(name:"stack-users(no filtering)", level:level, displayTable:displayTable, timeline:timeline, output:output & "/StackTargetUsers-NoFiltering.csv", filterSystemAccounts:false, filterComputerAccounts:false)

    # stack-users -t ../hayabusa/timeline.jsonl --level <level> --sourceUsers -o users.csv
    let cmd18 = StackUsersCmd(name:"stack-users(source)", level:level, displayTable:displayTable, timeline:timeline, output:output & "/StackSourceUsers.csv", sourceUsers:true, filterSystemAccounts:true, filterComputerAccounts:true)

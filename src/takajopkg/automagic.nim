...
proc autoMagic(level: string = "low", skipProgressBar:bool = false, displayTable:bool = false, output: string = "case-1", quiet: bool = false, timeline: string) =
    ...
    let cmd1 = ExtractScriptBlocksCmd(level: level, skipProgressBar: skipProgressBar, displayTable:displayTable, timeline: timeline, output: output & "/scriptblock-logs/")
    ...
    let cmd10 = StackIpAddressesCmd(name: "Stack IpAddresses", level:level, displayTable:displayTable, timeline:timeline, output:output & "/SourceIP-Addresses.csv")

    # stack-ip-addresses -t ../hayabusa/timeline.jsonl --level <level> --targetIpAddresses -o case-1/TargetIP-Addresses.csv
    let cmd11 = StackIpAddressesCmd(name: "Stack IpAddresses", level:level, displayTable:displayTable, timeline:timeline, output:output & "/TargetIP-Addresses.csv", targetIpAddresses:true)
    ...
    let cmd17 = StackUsersCmd(name:"Stack Users", level:level, displayTable:displayTable, timeline:timeline, output:output & "/TargetUsers-NoFiltering.csv")

    # stack-users -t ../hayabusa/timeline.jsonl --level <level> --sourceUsers -o users.csv
    let cmd18 = StackUsersCmd(name:"Stack Users", level:level, displayTable:displayTable, timeline:timeline, output:output & "/SourceUsers.csv", sourceUsers:true)
    ...
    let cmd19 = StackUsersCmd(name:"Stack Users", level:level, displayTable:displayTable, timeline:timeline, output:output & "/SourceUsers-NoFiltering.csv", sourceUsers:true, filterSystemAccounts:false, filterComputerAccounts:false)

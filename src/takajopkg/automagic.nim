...
proc autoMagic(level: string = "low", skipProgressBar:bool = false, displayTable:bool = false, output: string = "case-1", quiet: bool = false, timeline: string) =
    ...
    let cmd16 = StackUsersCmd(name:"stack-users(target)", level:level, displayTable:displayTable, timeline:timeline, output:output & "/TargetUsers.csv" , filterSystemAccounts:true, filterComputerAccounts:true)
    ...
    let cmd18 = StackUsersCmd(name:"stack-users(source)", level:level, displayTable:displayTable, timeline:timeline, output:output & "/SourceUsers.csv", sourceUsers:true, filterSystemAccounts:true, filterComputerAccounts:true)

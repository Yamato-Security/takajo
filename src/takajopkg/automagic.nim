...
proc autoMagic(level: string = "low", skipProgressBar: bool = false,
  ...
  for timelinePath in filePaths:
    ...
    let cmd1 = ExtractScriptBlocksCmd(name: "extract-scriptblocks",
        ...
        timeline: timelinePath, output: output & "/scriptblock-logs")
    ...
    let cmd2 = ListDomainsCmd(name: "list-domains", displayTable: displayTable,
        timeline: timelinePath, output: output & "/ListDomains.txt")
    ...
    let cmd3 = ListDomainsCmd(name: "list-domains(detailed)",
        displayTable: displayTable, timeline: timelinePath, output: output &
    ...
    let cmd4 = ListHashesCmd(name: "list-hashes", displayTable: displayTable,
        timeline: timelinePath, level: level, output: output & "/ListHashes")

    # list-ip-addresses -t ../hayabusa/timeline.jsonl -o case-1/IP-Addresses.txt
    let cmd5 = ListIpAddressesCmd(name: "list-ip-addresses",
        displayTable: displayTable, timeline: timelinePath, output: output & "/ListIP-Addresses.txt")
    ...
    let cmd6 = StackCmdlineCmd(name: "stack-cmdlines", level: level,
        displayTable: displayTable, timeline: timelinePath, output: output & "/StackCmdlines.csv")
    ...
    let cmd7 = StackComputersCmd(name: "stack-computers", level: level,
        displayTable: displayTable, timeline: timelinePath, output: output & "/StackTargetComputers.csv")
    ...
    let cmd9 = StackDNSCmd(name: "stack-dns", level: level,
        displayTable: displayTable, timeline: timelinePath, output: output & "/StackDNS.csv")
    ...
    let cmd12 = StackLogonsCmd(name: "stack-logons", displayTable: displayTable,
        timeline: timelinePath, output: output & "/StackLogons.csv")
    ...
    let cmd13 = StackProcessesCmd(name: "stack-processes", level: level,
        displayTable: displayTable, timeline: timelinePath, output: output & "/StackProcesses.csv")
    ...
    let cmd14 = StackServicesCmd(name: "stack-services", level: level,
        displayTable: displayTable, timeline: timelinePath, output: output & "/StackServices.csv")

    # stack-tasks -t ../hayabusa/timeline.jsonl --level <level> -o case-1/Tasks.csv
    let cmd15 = StackTasksCmd(name: "stack-tasks", level: level,
        displayTable: displayTable, timeline: timelinePath, output: output & "/StackTasks.csv")
    ...
    # timelinePath-logon -t ../hayabusa/timeline.jsonl -o case-1/LogonTimeline.csv
    let cmd20 = TimelineLogonCmd(name: "timelinePath-logon",
        displayTable: displayTable, timeline: timelinePath, output: output & "/TimelineLogon.csv")

    # timelinePath-partition-diagnostic -t ../hayabusa/timeline.jsonl -o case-1/PartitionDiagnosticTimeline.csv
    let cmd21 = TimelinePartitionDiagnosticCmd(
        name: "timelinePath-partition-diagnostic", displayTable: displayTable,
        timeline: timelinePath, output: output & "/TimelinePartitionDiagnostic.csv")

    # timelinePath-suspicious-processes -t ../hayabusa/timeline.jsonl --level <level> -o case-1/SuspiciousProcesses.csv
    let cmd22 = TimelineSuspiciousProcessesCmd(
        name: "timelinePath-suspicious-processes", level: level,
        displayTable: displayTable, timeline: timelinePath, output: output & "/TimelineSuspiciousProcesses.csv")

    # timelinePath-tasks -t ../hayabusa/timeline.jsonl -o case-1/TaskTimeline.csv
    let cmd23 = TimelineTasksCmd(name: "timelinePath-tasks",
        displayTable: displayTable, timeline: timelinePath, output: output & "/TimelineTask.csv")
    ...
    let cmd24 = TTPSummaryCmd(name: "ttp-summary", displayTable: displayTable,
        timeline: timelinePath, output: output & "/TTPSummary.csv")
    ...
    let cmd25 = TTPVisualizeCmd(name: "ttp-visualize",
        displayTable: displayTable, timeline: timelinePath, output: output & "/MitreTTP-Heatmap.json")
    ...
    let cmd = AutoMagicCmd(level: level, skipProgressBar: skipProgressBar,
        displayTable: displayTable, output: output, timeline: timelinePath,

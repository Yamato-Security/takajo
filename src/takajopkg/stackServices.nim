...
method filter*(self: StackServicesCmd, x: HayabusaJson): bool =
  return (x.EventID == 7045 and not self.ignoreSystem and x.Channel == "Sys") or
      (x.EventID == 4697 and not self.ignoreSecurity and x.Channel == "Sec")
...
proc stackServices(level: string = "informational", ignoreSystem: bool = false,

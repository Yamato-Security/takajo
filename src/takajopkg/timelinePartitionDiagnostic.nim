...
proc extractVSN(jsonLine: HayabusaJson): array[4, string] =
  ...
  if "PartitionStyle" notin jsonLine.ExtraFieldInfo or jsonLine.ExtraFieldInfo[
      "PartitionStyle"].getInt() != 0:
    return res
...
method resultOutput*(self: TimelinePartitionDiagnosticCmd) =
  var savedFiles = "n/a"
  var results = "n/a"
  let header = ["Timestamp", "Computer", "Manufacturer", "Model", "Revision",
      "SerialNumber", "VSN0", "VSN1", "VSN2", "VSN3"]
  if self.output != "":
    ...
    for t in self.seqOfResultsTables:
      table.add t[header[0]], t[header[1]], t[header[2]], t[header[3]], t[
          header[4]], t[header[5]], t[header[6]], t[header[7]], t[header[8]], t[
          header[9]]
    if self.displayTable:
      echo ""
...
proc timelinePartitionDiagnostic(skipProgressBar: bool = false,
